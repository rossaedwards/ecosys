...
Section ID : section_LIII
Figure ID  : section_LIII_fig53
Label      : Figure 53
...
def run_simulation(output_dir: Path | None = None) -> Path:
    """Generate stability envelope (V < V_max stable region)."""
    ...
    out_path = output_dir / "fig_053_053_stability_envelope.png"
    ...
    im = ax.pcolormesh(F, I, V, cmap="RdYlGn_r", shading="auto", vmin=0, vmax=0.8)
    ax.contour(F, I, V, levels=[0.05, 0.2, 0.5], colors="black", alpha=0.6)
    ax.set_xlabel(r"$rAE_f$")
    ax.set_ylabel(r"$rAE_i$")
    ax.set_title("Stability Envelope")
