...
use crate::{
    gov::BlissId,
    network::{NetworkError, NetworkResult, NodeManager},
    shard::ShardId,
};
use std::{
    collections::HashMap,
...
impl ReticulumBridge {
    ...
    pub fn new(node_manager: Arc<NodeManager>) -> Arc<Self> {
        ...
        lanes.insert(
            NetworkLane::Starlink,
            LaneStatus::new(NetworkLane::Starlink),
        );
    ...
    pub async fn send_data(
        ...
        use crate::network::packet::{HandshakePacket, NetworkPacket, PacketType};

        let lane = self.route_data(data, replication).await?;

        info!(
            "📤 Routing {} bytes to {} via {}",
            data.len(),
            target,
            lane.name()
        );
        ...
        if data.len() > 10_000_000 {
            ...
            let _hello_bytes = hello.serialize().map_err(|e| {
                NetworkError::ConfigError(format!("Packet serialization failed: {}", e))
            })?;
        ...
        // Serialize packet
        let packet_bytes = packet.serialize().map_err(|e| {
            NetworkError::ConfigError(format!("Packet serialization failed: {}", e))
        })?;
        ...
        info!(
            "📦 Packet serialized: {} bytes (header: {} bytes, payload: {} bytes)",
            packet_bytes.len(),
    ...
    pub async fn replicate_shard(
        ...
        let shard = shard_store.load_shard(shard_id).await.map_err(|e| {
    ...
    async fn detect_lora(&self) -> NetworkResult<bool> {
        ...
        {
            use std::process::Command;
            if let Ok(output) = Command::new("mode").output() {
                let output_str = String::from_utf8_lossy(&output.stdout);
        ...
        #[cfg(all(not(feature = "hardware-embedded"), target_os = "linux"))]
        {
            ...
            if let Ok(output) = Command::new("lsusb").output() {
                ...
                if output_str.contains("1d50") || // Seeed Studio
                   output_str.contains("0403")
                {
                    // FTDI (common for LoRa modules)
                    return Ok(true);
                }
            }
        }
...
impl From<ReticulumError> for NetworkError {
    fn from(err: ReticulumError) -> Self {
        match err {
            ...
            ReticulumError::NoActiveLanes => {
                NetworkError::ConfigError("No active network lanes available".to_string())
            }
        }
    }
}
