...
use crate::{
    gov::{BlissId, SoulACL},
    network::{mesh::Mesh, peer::PeerState},
    shard::{Shard, ShardId, ShardMetadata},
...
impl ReplicationEngine {
    ...
    pub async fn replicate_shard(
        ...
        shard: &Shard,
        soul: &BlissId,
    ) -> Result<ReplicationTask, ReplicationError> {
        ...
        let targets: Vec<_> = healthy_peers.choose_multiple(
            &mut rng,
            self.config.replication_factor.min(healthy_peers.len()),
        );
        ...
        self.active_tasks
            .write()
            .await
            .insert(shard_id.clone(), task.clone());
        ...
        let replication_futures = targets.iter().map(|peer_id| {
            ...
            async move { engine.replicate_to_peer(&shard_clone, &peer_id_clone).await }
        });
    ...
    pub async fn read_shard_quorum(&self, shard_id: &ShardId) -> Result<Shard, ReplicationError> {
        ...
        let read_futures = replicas
            .iter()
            .take(self.config.read_quorum)
            .map(|peer_id| {
                let engine = Arc::clone(&self);
                let shard_id_clone = shard_id.clone();
                async move { engine.read_from_peer(&shard_id_clone, peer_id).await }
            });
    ...
    async fn heal_shard_replicas(&self, shard_id: &ShardId, original_peers: &[BlissId]) {
        ...
        let missing_count = self
            .config
            .replication_factor
            .saturating_sub(original_peers.len());

        if missing_count > 0 && !healthy_peers.is_empty() {
            info!(
                "🩹 Healing shard {}: {} missing replicas",
                shard_id, missing_count
            );
    ...
    async fn select_healthy_peers(&self, soul: &BlissId) -> Result<Vec<BlissId>, ReplicationError> {
        ...
        let healthy: Vec<_> = peers
            ...
            .filter_map(|p| {
                ...
                (peer.is_active && peer.health_score() >= 80 && peer.acl.can_read(soul))
                    .then_some(peer.id.clone())
            })
            .collect();
    ...
    async fn replicate_to_peer(
        ...
        if shard.data.is_empty() {
            return Err(ReplicationError::InvalidShard(
                "Shard data is empty".to_string(),
            ));
        }

        if peer_id.0.as_bytes().is_empty() {
            return Err(ReplicationError::InvalidPeer(
                "Peer ID is empty".to_string(),
            ));
        }
        ...
        for attempt in 0..MAX_RETRIES {
            ...
            .await
            {
                ...
                Ok(Err(e)) if attempt < MAX_RETRIES - 1 => {
                    warn!(
                        "Replication failed (attempt {}/{}): {}, retrying...",
                        attempt + 1,
                        MAX_RETRIES,
                        e
                    );
                ...
                Err(_) if attempt < MAX_RETRIES - 1 => {
                    warn!(
                        "Replication timeout (attempt {}/{}), retrying...",
                        attempt + 1,
                        MAX_RETRIES
                    );
                    tokio::time::sleep(Duration::from_millis(500 * (attempt as u64 + 1))).await;
    ...
    async fn replicate_to_peer_internal(
        &self,
        shard: &Shard,
        peer_id: &BlissId,
    ) -> Result<(), ReplicationError> {
        ...
        for attempt in 0..MAX_RETRIES {
            ...
            .await
            {
                Ok(Ok(shard)) => {
                    ...
                    if shard.data.is_empty() {
                        return Err(ReplicationError::InvalidShard(
                            "Read shard data is empty".to_string(),
                        ));
                    }
    ...
    async fn read_from_peer_internal(
        &self,
        shard_id: &ShardId,
        peer_id: &BlissId,
    ) -> Result<Shard, ReplicationError> {
...
mod tests {
    ...
    async fn test_replication_quorum() {
        ...
        let task = engine
            .replicate_shard(&shard, &BlissId::genesis())
            .await
            .unwrap();
