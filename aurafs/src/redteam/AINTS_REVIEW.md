# AINTS_REVIEW — `aurafs/src/redteam/`
...
- Name matrix: chaos, exploit, net, gov, audit_simulator, quantum_breaker, fuzzers, `cli.rs`.
- `mod.rs` authorized-testing comment; `run_test_suite()` placeholder zeros.
- `pinktribesuite.md` still missing at `src/pinktribesuite.md`.

**Wired:** **No.** Not in `lib.rs`. No `security-tools` Cargo feature.  
**Era / voice:** Mostly 14-line stubs; a few large Love+QDiv named files. 2025-12-28 inventory txt.

## Suggestions

- Keep isolated from default crate.
- Do not complete stubs or write attack procedures.
- If Ross wants a suite doc, it is inventory/scoring only — no payloads.

## Clarifying questions needed

### Architectural
- Defensive tests only, or a game/suite brand (Pink Tribe)?

### Framework
- Not SAGES. Not FTQC cryptanalysis.

### Vision
- Training ground for SAGES, or never-ship lab?

### Intentions
- Filenames are labels, not a build list.

### Design
- Do not enable from `lib.rs` until a defensive-test charter exists.

### Ideas
- Pair names with `whitehat/` 1:1 as a documentation suite only.
...
|---|---|
| **TSLCA** (`tslca/`) | N/A. |
| **FTQC** (`ftqc/`) | quantum_breaker names are not FTQC experiments. |
| **TVFD** (`tvfd/`) | N/A. |
| **SAGES** (`sages/`, `SAGES_Framework/`) | Immune-system story lives in welcome/SAGES, not this tree. |
| **Fuxyez** (`fuxyez/`) | N/A. |
