...
use crate::{
    gov::{BlissId, SoulACL},
    shard::{Shard, ShardId, ShardManager, ShardMetadata, metadata::LatticeGeometry},
    storage::{
        directory::Directory,
        fuse::AuraFSFuse,
        inode::{Inode, InodeId, InodeTimestamps},
        journal::{FsEvent, QuantumJournal},
        local::LocalShardStorage,
        quota::{SoulQuota, SoulQuotaManager},
        shard::{ShardStore, ShardStoreConfig},
        snapshot::{SnapshotId, SnapshotManager},
    },
};
use dashmap::DashMap;
...
impl AuraFS {
    ...
    pub async fn forge(config: FsConfig) -> Result<Arc<Self>, FsError> {
        ...
        let acl_enforcer = Arc::new(crate::shard_server::acl::AclEnforcer::new(
            crate::crypto::quantum::KyberKeypair::generate().unwrap(),
        ));

        // 2. Initialize Storage Stack (Physics + Middleware)
        let local_storage =
            LocalShardStorage::forge(config.storage_path.clone(), quotas.clone(), journal.clone())
                .await
    ...
    async fn bootstrap_root(&self) -> Result<(), FsError> {
        ...
        self.posix_to_inode
            .insert(self.root_ino, root_inode_id.clone());
        self.inode_to_posix
            .insert(root_inode_id.clone(), self.root_ino);

        // Store root inode (Persist to Lattice)
        self.shard_store
            .store_inode(&root_inode, &BlissId::genesis())
            .await
            .map_err(|e| FsError::ResourceExhausted(e.to_string()))?;
        ...
        self.journal
            ...
            .await
            .map_err(|e| FsError::JournalError(e.to_string()))?;
    ...
    pub async fn resolve_path(&self, parent_ino: u64, name: &str) -> Result<InodeId, FsError> {
        if *self.shutdown.read().await {
            return Err(FsError::InvalidOperation(
                "Filesystem is shutting down".to_string(),
            ));
        }
    ...
    pub async fn link_child(
        ...
        self.shard_store
            .store_inode(&parent_inode, &BlissId::genesis())
            .await
            .map_err(|e| FsError::ResourceExhausted(e.to_string()))?;
    ...
    pub async fn update_inode(&self, inode: &Inode) -> Result<(), FsError> {
        self.shard_store
            .store_inode(inode, &BlissId::genesis())
            .await
            .map_err(|e| FsError::ResourceExhausted(e.to_string()))
    }
}
...
impl ShardStore {
    pub async fn store_inode(
        &self,
        inode: &Inode,
        soul: &BlissId,
    ) -> Result<ShardId, crate::storage::shard::ShardStoreError> {
    ...
    pub async fn load_inode(
        &self,
        id: &InodeId,
    ) -> Result<Inode, crate::storage::shard::ShardStoreError> {
        ...
        let inode: Inode = bincode::deserialize(&shard.data).map_err(|_| {
            crate::storage::shard::ShardStoreError::ErasureError("De-ser failed".into())
        })?;
        Ok(inode)
    }
}
