# AINTS_SUPPZ — `aurafs/src/compression/`
...
Love + Quantum Division. LZ4, Zstd, “quantum wavelet” compressor, manager, stats. **`config.rs` is empty.** `mod.rs` is TRL-4-adjacent (locked files include `lattice.rs` and `manager.rs`).

## Wired?

**Yes** as `pub mod compression`. Cargo features `zstd-compression` / `lz4-compression` are optional; code may not match feature gates.

## Locks

`src/compression/lattice.rs` and `manager.rs` are `[modules.validated]` **ai_compression**. Public API needs `PHYSICS OVERRIDE`.

Do not treat lattice compression as TSLCA \(\mathcal{F}\) or SUXS-IFO \(\mathcal{U}\).

## Honest status

Real LOC. Empty `config.rs` + `mod.rs` listing `config` / `tests` is a compile risk. Codecs are product storage, not VASP audio.
