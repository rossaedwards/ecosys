...
- `[New]` VASP voting button — 3×3 lattice, up to 13 questions per pillar (`VibeTribe_Pillarz.md`)
- `[New]` Harvest → DSP → Tribe overlay promotion pipeline
...
- `[New]` Compare-your-vibe-to-friends — social Valence/Arousal comparison on a shared track
- `[New]` Collaborative playlists with group-average vibe compatibility scoring

**Snapchat integrations** (Snap Kit / Camera Kit / Lens Studio — AR, not VR; Snap doesn't have a VR surface)

- `[New]` AR Vibe Lens (Camera Kit) — a Lens Studio–built Lens embedded directly in-app via Camera Kit, rendering the current track's Photometric hex + Kinetic pulse as a live AR camera overlay; no Snapchat install required for the viewer
- `[New]` Public Lens on Snapchat — the same Lens published through Lens Studio as a standalone public Lens, discoverable in Snapchat's own Lens carousel/Snapcode, independent of the VAP install base
- `[New]` Vibe Card → Creative Kit share — the existing Vibe Card sent as a real pre-built Snap (sticker + backdrop + deep link back to the track) via Creative Kit instead of a generic OS share sheet
- `[New]` Vibez on the Snap Map — a location-tagged "what I'm vibing to" Story post via Creative Kit; surfaces on the poster's own Snap Map presence through Snapchat's native location-Story mechanism (no documented third-party API writes a pin directly onto the Map, so this rides that mechanism rather than a bespoke one)
- `[New]` Sign in with Snapchat + Bitmoji Tribe profile — Login Kit as an auth option; Bitmoji Kit pulls the user's own avatar into their Tribe contribution profile, posed per their dominant listening archetype
- `[New]` Landmarker festival Lenses — location-anchored Lenses (Lens Studio Landmarker + Location Cloud) that only unlock at a specific venue, a direct extension of the Vibe Fest concept from the ecosystem brainstorm
- `[New]` Commerce Kit storefront for Skinz — Snap's Commerce Kit lets Lenses sell digital goods directly inside the AR experience itself; a second, native storefront for Skinz-style unlockables alongside the in-app marketplace
- `[New]` Spectacles-ready by default — anything built in Lens Studio for Camera Kit deploys to Snap's Spectacles AR glasses from the same project with no extra build work, if that's ever a direction worth taking
