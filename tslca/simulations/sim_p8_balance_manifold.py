...
title: TSLCA Phase 8 — Balance Manifold
description: Probes symmetry index, tension gradient, and equilibrium distance of the Balance Manifold that bounds lawful lattice trajectories.
...
def run(seed: int = 369, steps: int = 1) -> dict[str, Any]:
    ...
    means = np.array([
        float(np.mean(fields["C"])),
        float(np.mean(fields["R"])),
        float(np.mean(fields["A"])),
    ])
    symmetry = float(np.clip(1.0 - np.std(means) / (np.mean(means) + 1e-9), 0, 1))
    tension = float(np.mean(np.abs(neighbor_mean(hif) - hif)))
    eq = abs(float(np.mean(hif)) - 0.55)
    return {
        "okf": OKF,
