...
title: TSLCA Phase 3 — Harmonic Integrity Field
description: Computes HIF = cbrt(C·R·A)·Φ(C,R,A) on the 27-node volume and enforces the Triple Threshold Gate as the root lattice invariant.
...
def run(seed: int = 369, steps: int = 1) -> dict[str, Any]:
    ...
    return {
        "okf": OKF,
        "summary": summarize(fields["C"], fields["R"], fields["A"], hif),
        "phi_lit": int(np.sum(phi)),
        "thresholds": THRESHOLDS,
        "layer_hif": layer_mean(hif),
        "seed": seed,
        "steps_ignored": steps,
    }
