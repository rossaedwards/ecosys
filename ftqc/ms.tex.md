% ============================================================================
% FTQC combined public manuscript (A = ms.tex / ftqc_zenodo.pdf)
% Lab sources left intact: prx_manuscript.tex (B), manuscript.tex (C)
% ============================================================================
...
\begin{abstract}
Current quantum computing architectures face scaling limits from decoherence ($T_2 < 100~\mu$s for superconducting qubits) and error-correction overhead ($\sim 1000:1$ physical-to-logical). This paper presents Fractal-enhanced Topological Quantum Computing (FTQC): qubits on Sierpi\'{n}ski gaskets (Hausdorff dimension $\Df = \log 3/\log 2 \approx 1.585$) access Hilbert spaces $\dim(\Hilbert_{\mathrm{acc}}) = d^{n \cdot \Df^{\alpha(k)}}$. With the hierarchical coupling $\alpha(k) = \log(1+k\eta)/\log \Df$, the $n=12$ corollary is $\mathcal{A}(12,3) \approx 10^{8.3}$ versus a Euclidean tensor product. Non-semisimple TQFTs, via neglecton braiding ($d_\omega = 0$), give a universal gate set with $\sim 16\times$ fewer gates than magic-state distillation. Two decoherence ratios are kept distinct: the fractal-embedded FTQC architecture has $\gamma_{\mathrm{FTQC}}/\gamma_{\mathrm{Euclidean}} = 0.063$ ($\sim 16\times$ $T_2$); a 19-site $C_{6v}$ photonic cell without that embedding has $\gamma_{19}/\gamma_{\mathrm{Euclidean}} = 0.63$ ($\sim 1.6\times$ $T_2$). Qiskit simulations give a $7.1\times$ Hilbert advantage at $n=5$, GHZ fidelity $0.912$, and state purity $\TraceOp(\rho^2)=1$. Four proposed laboratory protocols test the claims; program cost is a proposed validation budget, not the identity of the architecture.
\end{abstract}
...
\subsection{** APS-FTQC-001 **}\label{aps-ftqc-001}
...
Surface-code fault tolerance at logical error $10^{-12}$ wants code distance $d \geq 27$, or $\sim 2d^2 \approx 1458$ physical qubits per logical qubit~\cite{Fowler2012}. That overhead is the scaling problem FTQC attacks---not by minting a corporate brand as an architecture, but by changing the lattice and the anyon category.

FTQC is the architecture. Aurphyx LLC is the affiliation and the Aurphyx Primordial Standard is the organism header law. The three working advantages are:

\begin{enumerate}
\item \textbf{Hilbert space scaling} (Sec.~\ref{sec:hilbert_scaling}): fractal lattices enlarge the accessible state space at fixed node count.
\item \textbf{Topological universality} (Sec.~\ref{sec:tqft}): neglecton braiding completes a dense gate set without magic-state distillation.
\item \textbf{Decoherence suppression} (Sec.~\ref{sec:photonics}): $C_{6v}$ photonic engineering plus, separately, fractal embedding. The two $\gamma$ ratios are not interchangeable.
\end{enumerate}

The three $\sim 16\times$ numbers in this paper are different objects. \emph{Gate overhead} is neglecton braiding versus magic-state distillation. \emph{Error-correction overhead} is $R_{\mathrm{Euclid}}/R_{\mathrm{fractal}} \approx 1458/89$. \emph{$T_2$ / $\gamma$} for the fractal-embedded architecture is $1/0.063$. Do not collapse them.
...
\end{equation}
and locks $\alpha(k)$ and the $n=12$ corollary there.
...
Hexagonal $C_{6v}$ lattices~\cite{Yablonovitch1987,John1987,Joannopoulos2008} open a complete TM gap $\Delta\omega/\omega_{\mathrm{mid}} = 21\%$ and support flatbands. Anderson localization on a fractal-embedded sublattice is the $0.063$ claim. The same 19-site cell, simulated without that embedding, is the $0.63$ claim. Both stay; they are labeled in Sec.~\ref{sec:photonics}.

\subsection{Organization}

Section~\ref{sec:hilbert_scaling} proves Hilbert scaling. Section~\ref{sec:tqft} treats neglecton universality and Qiskit evidence. Section~\ref{sec:photonics} is photonic engineering, including the 19-site geometry, PWE, Lindblad, and fabrication harvested from the photonic manuscript. Section~\ref{sec:experiments} is a proposed four-protocol validation program. Section~\ref{sec:discussion} compares platforms (projected FTQC rows only). Section~\ref{sec:conclusion} closes.

% ============================================================================
\section{Fractal Hilbert Space Scaling}
\label{sec:hilbert_scaling}

% LOCKED α(k): α(k) = log(1+kη)/log D_f from section_ii / prx_manuscript.tex.
% Reason: that is the written proof (hierarchical product + geometric series).
% The prior ms.tex form α(k)=1-c/k was a three-line sketch only; keeping it
% would break the dim(H_acc)=d^{n D_f^{α(k)}} corollary in section_ii.
% LOCKED n=12: A(12,3) ≈ 10^{8.3} (η=0.7, α(3)≈2.38, dim ≈ 2^{39.4}).
% The older 10^4 figure was an unaligned abstract leftover, not this corollary.
...
\begin{remark}
Photonic evanescent coupling typically sits at $\eta \approx 0.5$--$0.8$. Direct waveguide links can approach $\eta \to 1$.
\end{remark}
...
\end{equation}
This is the locked $n=12$ number for the combined manuscript.
\end{example}

\begin{table}[h]
\caption{Sierpi\'{n}ski Hilbert scaling ($d=2$, $\eta=0.7$) from the locked $\alpha(k)$.}
\label{tab:scaling}
\begin{ruledtabular}
...
A low-depth check, not a second corollary: at $k=1$ the naive factor $2^{n(\Df-1)}$ gives $2^{5\times 0.585}\approx 7.1$ at $n=5$, which is what the Qiskit circuit reports. Full hierarchical coupling at $k=3$ is Table~\ref{tab:scaling}, not $10^4$.

\subsection{Error-correction overhead (not gate overhead, not $T_2$)}
...
At $p_{\mathrm{phys}}=10^{-3}$, $p_L=10^{-12}$: $R_{\mathrm{Euclidean}}\approx 1458$ versus $R_{\mathrm{fractal}}\approx 89$---a $16\times$ \emph{error-correction} overhead cut. That is not the neglecton gate-count factor and not $1/\gamma_{\mathrm{FTQC}}$.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{Fig1_Hilbert_Scaling}
\caption{Hilbert-space advantage $\mathcal{A}(n,k)$ for Sierpi\'{n}ski ($k=3$) versus Euclidean. The $n=5$ Qiskit point is $7.1\times$; the locked $n=12$ corollary is $10^{8.3}$.}
...
If a non-semisimple MTC contains a neglecton with $R_{\omega,a}\neq\pm 1$ for some simple $a$, then semisimple braiding plus neglecton braiding is dense in $\mathrm{SU}(N)$. For $(\mathfrak{sl}(2),k=2)$ this is $\sim 16\times$ \emph{fewer gates} than magic-state distillation---gate overhead, not EC overhead, not $T_2$.
\end{theorem}
...
Scaling advantage & $7.1\times$ & matches $k=1$ check \\
$n=12$ theorem target & $10^{8.3}$ & Table~\ref{tab:scaling}, not $10^4$ \\
\end{tabular}
\end{ruledtabular}
\end{table}

The $16\times$ \emph{fidelity stack} in the TQFT section is a product of three different mechanisms ($\sim 3\times$ non-Abelian protection, $\sim 2\times$ no T-gate overhead, $\sim 2.7\times$ from Proposition~\ref{prop:qec}). It is not a fourth copy of $\gamma_{\mathrm{FTQC}}$.
...
This section is one photonic chapter, not a second paper. The $C_{6v}$ band-structure and fractal-embedded $\gamma$ come from the FTQC spine and Sec.~IV sources. The 19-site geometry, tight-binding hoppings, Lindblad run \emph{without} fractal embedding, and fused-silica write protocol are harvested from the photonic manuscript.
...
\end{equation}
with $\omega\in[2.50,3.10]\times 2\pi c/a$. Flatbands 5--6 near $\omega=3.50\times 2\pi c/a$ have $v_g<0.01c$. Dirac cones sit at $K,K'$.
...
\subsection{Two $\gamma$ ratios (required split)}
\label{subsec:gamma-split}

\textbf{Ratio A --- fractal-embedded FTQC architecture} (spine / PRX / Sec.~IV).
...
\label{eq:gamma_ftqc}
\end{equation}
Numerical check: $0.058\pm 0.012$, consistent with $0.063$. This is the architecture claim.

\textbf{Ratio B --- 19-site cell without that embedding} (photonic manuscript).
...
\label{eq:gamma_19}
\end{equation}
Extracted $1/e$ times: $T_2^{\mathrm{(bulk)}}=47~\mu$s, $T_2^{(C_{6v})}=62~\mu$s, $T_2^{\mathrm{(fractal\ sublattice\ on\ 19\ sites)}}=75~\mu$s. Band gap alone $\approx 1.32\times$; adding a fractal \emph{sublattice on the 19-site cell} adds $\approx 1.21\times$. That is not Eq.~\eqref{eq:gamma_ftqc}.
...
\caption{Anderson localization on the Sierpi\'{n}ski gasket. $d_s=1.36<2$ forces localization independent of disorder strength---the mechanism behind $\gamma_{\mathrm{FTQC}}$, not $\gamma_{19}$.}
...
\caption{Coherence dynamics for the fractal-embedded architecture, consistent with $\gamma_{\mathrm{FTQC}}/\gamma_{\mathrm{Euclidean}}=0.063$ ($\sim 16\times$ $T_2$). Do not read this figure as the 19-site $1.6\times$ Lindblad run.}
\label{fig:coherence}
\end{figure}

\subsection{Topological edge states}

$C_{6v}$ edge states: chiral, $v_g\approx 0.08c$, $\xi_{\mathrm{edge}}\approx 2a$, robust for $\delta r/r<10\%$. These are interconnects, not a third $\gamma$.

\subsection{Femtosecond fabrication (photonic manuscript)}
\label{subsec:fabrication}

Proposed write in fused silica~\cite{Davis1996,SzameitNolte2010}: piranha-clean wafers; Ti:sapphire $800$~nm, $50$--$100$~fs, $1$~kHz, $100\times$ oil NA $1.25$, $1$--$10$~mm\,s$^{-1}$, $50$--$200$~nJ; optional $5\%$ HF etch for air holes; $600^\circ$C anneal. Characterization: broadband stop-band spectroscopy, NSOM at the flatband, controlled pulse-energy disorder for $w(z)$, HOM visibility for photonic $T_2$. This is Protocol~3's shop drawing. Itemized photonic-only shop cost in the source manuscript is $\sim\$105$K; the combined program table in Sec.~\ref{sec:experiments} uses the four-protocol proposed budget.
...
P2's $\geq 10\times$ success bar is a conservative laboratory target. The locked theorem number at $n=12$, $k=3$ remains $10^{8.3}$.
...
The projected $T_2=1.6$~ms is $100~\mu$s scaled by $1/0.063$ (Ratio A). It is not the 19-site $1.6\times$ Lindblad factor. The $\sim 60:1$ EC ratio is Proposition~\ref{prop:qec}, not neglecton gate count.
...
(1) Theorem~\ref{thm:main} assumes a hierarchical $H$ that physical devices only approximate. (2) Neglectons are not yet a condensed-matter fact. (3) $\gamma_{\mathrm{FTQC}}=0.063$ applies to dephasing that couples through the photonic mode profile; $\gamma_{19}=0.63$ is the 19-site Lindblad without the full embedding. (4) Fabrication past $k>4$ is hard. (5) Full 3D FDTD for out-of-plane loss is still open (photonic manuscript).
...
FTQC is the architecture: fractal Hilbert scaling, neglecton universality, and photonic isolation. Aurphyx LLC is the affiliation. Locked results:

\begin{enumerate}
\item $\dim(\Hilbert_{\mathrm{acc}})=d^{n\cdot\Df^{\alpha(k)}}$ with $\alpha(k)=\log(1+k\eta)/\log\Df$. At $n=12$, $k=3$, $\eta=0.7$: $\mathcal{A}\approx 10^{8.3}$.
\item Neglecton braiding: $\sim 16\times$ \emph{gate} overhead cut versus magic-state distillation.
\item Error correction: $\sim 16\times$ cut in $R$ ($1458\to 89$).
\item $\gamma_{\mathrm{FTQC}}/\gamma_{\mathrm{Euclidean}}=0.063$ (embedded architecture). $\gamma_{19}/\gamma_{\mathrm{Euclidean}}=0.63$ (19-site cell without that embedding).
\item Qiskit: $7.1\times$ at $n=5$, GHZ $0.912$, purity $\TraceOp(\rho^2)=1$.
\item Four proposed protocols; $\approx\$1.25$M / TRL-4 is a proposed budget, not the paper's name.
\end{enumerate}
