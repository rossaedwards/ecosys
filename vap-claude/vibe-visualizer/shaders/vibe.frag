...
/* ================================================================
   ...
   Mechanical port of shaders/vibe.frag (GLSL 120 -> 330 core):
     - #version 120 -> #version 330 core
     - gl_FragColor -> out vec4 fragColor
   No logic below this header changed. chladni() is SACRED — untouched.
   ================================================================ */
...
/* Phase 7 — Nexus Vibez: TSL (Three-Squared-Lattice) extension.
   Adds to the Chladni field; never replaces it. */
uniform float u_tsl_x;        /* sub-bass lattice anchor strength 0-1 */
uniform float u_tsl_y;        /* mid harmonic oscillation 0-1         */
uniform float u_tsl_z;        /* high particle emissivity 0-1         */
uniform float u_phase_align;  /* L/R stereo phase coherence -1 to +1  */
uniform float u_lattice_rot;  /* accumulated phase rotation (radians) */
...
void main() {
    ...
    /* ── PHASE 7: NEXUS VIBEZ → TSL lattice adds to the field ─────── */
    float nexus = nexus_vibez(uv, u_tsl_x, u_tsl_y, u_tsl_z, u_phase_align, u_lattice_rot);
    field += nexus * 0.6;
