...
class Axiom(BaseModel):
    ...
    formal_expression: Optional[str] = Field(
        None, description="Mathematical / formal notation"
    )
    active_in_projects: List[str] = Field(default_factory=list)
...
class ProjectMeta(BaseModel):
    ...
    priority: int = Field(
        default=5, ge=1, le=10, description="1=highest focus, 10=background"
    )
    dependencies: List[str] = Field(
        default_factory=list, description="Project keys this depends on"
    )
    integrates_with: List[str] = Field(
        default_factory=list, description="Project keys this embeds into"
    )
...
class EpisodicMemory(AurphyxBase):
    ...
    model_name: Optional[str] = Field(
        None, description="Specific model ID if known, e.g. 'sonar-pro'"
    )
    turn_index: int = Field(default=0, ge=0)
    parent_id: Optional[str] = Field(
        None, description="Prior turn ID for conversation threading"
    )
    tags: List[str] = Field(default_factory=list)
    intent: Optional[str] = Field(
        None, description="Detected user intent / topic label"
    )
    sentiment: Optional[float] = Field(
        None, ge=-1.0, le=1.0, description="-1.0 negative | 0.0 neutral | 1.0 positive"
    )
    embedding_id: Optional[str] = Field(
        None, description="Qdrant point UUID after indexing"
    )
...
class SemanticMemory(AurphyxBase):
    ...
    project: str
    category: str = Field(
        ..., description="Knowledge category, e.g. 'architecture', 'physics'"
    )
    subcategory: Optional[str] = None
    content: str
    summary: Optional[str] = Field(
        None, description="1-sentence fast-retrieval summary"
    )
    tags: List[str] = Field(default_factory=list)
    domains: List[ProjectDomain] = Field(default_factory=list)
...
class MetaMemory(AurphyxBase):
    ...
    verified_by: Optional[str] = Field(
        None, description="LLM provider or human reviewer"
    )
    verified_at: Optional[datetime] = None
    deprecated: bool = False
    deprecated_reason: Optional[str] = None
    superseded_by: Optional[str] = Field(
        None, description="ID of the replacing MetaMemory"
    )
...
class QuantumMemory(AurphyxBase):
    ...
    lattice_type: Optional[str] = Field(
        None, description="e.g. 'Sierpinski', 'Majorana'"
    )
    qubit_count: Optional[int] = Field(None, ge=1)
    error_rate: Optional[float] = Field(None, ge=0.0, le=1.0)
    coherence_time_us: Optional[float] = Field(
        None, description="Coherence time in microseconds"
    )
    topological_invariant: Optional[float] = Field(
        None, description="Chern number or Z2 invariant"
    )
    hamiltonian_ref: Optional[str] = Field(
        None, description="File path, arXiv ID, or DOI"
    )
    observable: Optional[str] = Field(
        None, description="Primary observable being tracked"
    )
...
class CreativeMemory(AurphyxBase):
    ...
    universe: Optional[str] = Field(
        None, description="Canonical lore universe this belongs to"
    )
    characters: List[str] = Field(default_factory=list)
    themes: List[str] = Field(default_factory=list)
    dualities_expressed: List[str] = Field(
        default_factory=list, description="Dualities this work embodies"
    )
    soul_id_ref: Optional[str] = Field(None, description="BlissID / SoulHash reference")
    aurafs_shard_ref: Optional[str] = Field(
        None, description="AuraFS shard ID if minted"
    )
    tags: List[str] = Field(default_factory=list)
    status: Literal["draft", "in_progress", "complete", "published", "archived"] = (
        "draft"
    )
...
class GovernanceMemory(AurphyxBase):
    ...
    proposer: Optional[str] = Field(
        None, description="BlissID, soul hash, or entity identifier"
    )
    guardian_ids: List[str] = Field(
        default_factory=list, description="SAGES guardian IDs involved"
    )
    ...
    blissid_refs: List[str] = Field(
        default_factory=list, description="Participating BlissIDs"
    )
    ...
    def _enforce_immutability(self) -> "GovernanceMemory":
        ...
        if self.immutable and not self.ledger_hash:
            raise ValueError(
                "immutable GovernanceMemory records must provide a ledger_hash."
            )
        return self
...
class SoulProfile(BaseModel):
    ...
    soul_hash: str = Field(
        ..., description="SoulHash — immutable biometric fingerprint"
    )
    bliss_id: str = Field(
        ..., description="BlissID — one soul, one voice, one vote, one ID"
    )
    guardian_hash: Optional[str] = Field(
        None, description="GuardHash — guardian-layer verification token"
    )
    soul_chart_ref: Optional[str] = Field(
        None, description="SoulChart path or AuraFS shard reference"
    )
    ...
    quantum_entropy_seed: Optional[str] = Field(
        None, description="Quantum entropy seed — store offline only"
    )
    arora_profile_id: Optional[str] = Field(
        None, description="Arora OS user profile UUID"
    )
    aethornyx_avatar_id: Optional[str] = Field(
        None, description="In-game avatar ID in Aethornyx"
    )
    created_at: datetime = Field(default_factory=_now)
...
class AuraFSShard(BaseModel):
    ...
    aurafs_path: Optional[str] = Field(
        None, description="Logical path within AuraFS namespace"
    )
    pinned: bool = Field(
        default=False, description="True = never evicted from hot tier"
    )
    created_at: datetime = Field(default_factory=_now)
...
class ThreadSummary(BaseModel):
    ...
    decisions_made: List[str] = Field(
        default_factory=list, description="Concrete decisions reached"
    )
    open_questions: List[str] = Field(
        default_factory=list, description="Unresolved follow-up items"
    )
    ...
    soul_id_ref: Optional[str] = Field(
        None, description="BlissID of primary participant"
    )
    aurafs_archive_shard: Optional[str] = Field(
        None, description="AuraFS shard ID if archived"
    )
