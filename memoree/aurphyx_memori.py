...
class AurphyxMemori:
    ...
    def store_conversation(self, role: str, content: str, llm: str = "perplexity"):
        ...
        session_id = 1
        cur.execute("SELECT id FROM memori_session WHERE id = ?", (session_id,))
        if not cur.fetchone():
        ...
        conv_id = 1
        cur.execute("SELECT id FROM memori_conversation WHERE id = ?", (conv_id,))
        if not cur.fetchone():
    ...
    def get_conversation_history(
        self, limit: int = 10, llm: Optional[str] = None
    ) -> List[Dict]:
    ...
    def get_project_knowledge(
        self, project: str, category: Optional[str] = None
    ) -> List[Dict]:
        ...
        rows = cur.fetchall()
        conn.close()
        return [{"content": r[0], "created": r[1], "updated": r[2]} for r in rows]
    ...
    def store_procedural_memory(
        self, task: str, steps: List[str], success: bool = True
    ):
        """Alias for store_workflow (for bootstrap compatibility)"""
        success_rate = 1.0 if success else 0.0
        self.store_workflow(
            task, steps, success, frequency=1, success_rate=success_rate
        )
    ...
    def get_workflows(self, task_filter: Optional[str] = None) -> List[Dict]:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT id FROM memori_entity WHERE external_id = ?", ("workflows:all",)
        )
        result = cur.fetchone()
        if not result:
            conn.close()
    ...
    # === META MEMORY (Facts) ===
    def store_fact(
        self, fact: str, confidence: float, sources: List[str], verified: bool = False
    ):
        ...
        conn.close()
        print(
            f"✓ Stored fact: {fact[:50]}... (confidence: {confidence}, verified: {verified})"
        )
    ...
    def get_facts(self, verified_only: bool = False) -> List[Dict]:
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT id FROM memori_entity WHERE external_id = ?", ("facts:all",)
        )
        result = cur.fetchone()
        if not result:
            conn.close()
    ...
    def load_context_info(self, context_file: str = "Aurphyx_Context_info.txt"):
        ...
        try:
            ...
            print(
                f"📥 Loaded {context_file} into memory! (28 projects, EIN, persona ingested)"
            )
...
def test_memori():
    ...
    memori.store_conversation(
        "user", "I FUCKING HATE RE-EXPLAINING THINGS.", llm="perplexity"
    )
    memori.store_conversation(
        "assistant", "Groundhog Day loop ENDS NOW.", llm="perplexity"
    )
    ...
    memori.store_workflow(
        "create repo structure", ["Define directories", "Create files", "Add .gitkeep"]
    )
    memori.store_fact("Aurphyx LLC EIN: 41-3437055", 1.0, ["Aurphyx_Context_info.txt"])
